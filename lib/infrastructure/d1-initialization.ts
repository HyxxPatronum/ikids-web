...
export async function initializeD1Production(options: D1InitializationOptions) {
  ...
  return initializeProduction({
    ...
    async rebuildCatalog() {
      ...
      for (const card of seeded) seedSql.push(`INSERT INTO cards (id,slug,series_id,course_id,topic,theme,day,level,title,big_question,article_structure,image,content_json,status,updated_at) VALUES ('${escapeSql(card.cardId)}','${escapeSql(card.slug)}','${escapeSql(card.seriesId || card.courseId)}','${escapeSql(card.courseId)}','${escapeSql(card.topic)}','${escapeSql(card.theme)}',${Number(card.day || 0)},'${escapeSql(card.level)}','${escapeSql(card.title)}','${escapeSql(card.bigQuestion)}','${escapeSql(card.articleStructure)}','${escapeSql(card.image_file || card.image)}','${escapeSql(JSON.stringify(card))}','${escapeSql(card.status)}','${escapeSql(now)}') ON CONFLICT(id) DO UPDATE SET slug=excluded.slug,series_id=excluded.series_id,course_id=excluded.course_id,topic=excluded.topic,theme=excluded.theme,day=excluded.day,level=excluded.level,title=excluded.title,big_question=excluded.big_question,article_structure=excluded.article_structure,image=excluded.image,content_json=excluded.content_json,status=excluded.status,updated_at=excluded.updated_at;`);
      ...
      statements.push(
        'DELETE FROM published_vocabulary_terms;',
        'INSERT INTO published_vocabulary_terms (card_id,lexeme,surface_form,meaning,image,media_json,membership,source_slug,source_title,source_theme,source_image,updated_at) SELECT card_id,lexeme,surface_form,meaning,image,media_json,membership,source_slug,source_title,source_theme,source_image,updated_at FROM published_vocabulary_terms_staging;',
        `INSERT INTO infrastructure_state (name,value,updated_at) VALUES ('catalog','${terms.length}','${escapeSql(now)}') ON CONFLICT(name) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at;`,
      );
