...
async function handle(request:Request,context:RouteContext){
  ...
  if(path==='/words'&&method==='GET'){const rows=await db.prepare("SELECT content_json FROM cards WHERE status='published'").all();const map=new Map<string,any>();for(const row of rows.results||[]){const card:any=JSON.parse(String((row as any).content_json));for(const word of card.word_bank||[]){const key=String(word.english||'').toLowerCase();if(!key)continue;const entry=map.get(key)||{english:word.english,meaning:word.chinese||word.meaning||'',sources:[]};entry.sources.push({cardId:card.cardId,slug:slugFor(card),title:card.title,theme:card.theme});map.set(key,entry);}}return json({words:[...map.values()].sort((a,b)=>a.english.localeCompare(b.english))});}
