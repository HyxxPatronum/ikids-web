...
function playDeviceSpeech(text: string, region: 'us' | 'uk') {
...
function playPronunciation(text: string, region: 'us' | 'uk', hasRecording: boolean) {
  if (!hasRecording) {
    playDeviceSpeech(text, region);
    return;
  }
  let fellBack = false;
  const fallback = () => {
  ...
  const params = new URLSearchParams({ word: text, region });
  const audio = new Audio(`/api/pronunciation?${params}`);
    ...
    <aside className="react-dictionary-drawer" ref={drawerRef} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="dictionary-title">
      ...
      <div className="react-dictionary-content">
        ...
        {error ? <div className="state-box"><strong>查询未完成</strong><span>{error}</span><button className="btn blue" type="button" onClick={() => setRetry(value => value + 1)}>重新查询</button></div> : !data ? <div className="react-dictionary-loading"><h2>{activeScope}</h2><div className="skeleton" /></div> : <>
          <div className="react-dictionary-hero"><div><span className="badge">{data.catalogMembership || '参考词汇'}</span><div className="react-dictionary-word"><h2>{data.selectedScope || data.word}</h2>{data.phonetic && <span>{data.phonetic}</span>}</div><div className="react-pronunciation-row">{(['us', 'uk'] as const).map(region => { const pronunciation = data.pronunciations?.find(item => item.region === region); return <button key={region} className="react-pronunciation" type="button" title={pronunciation?.audio ? `播放${region === 'us' ? '美音' : '英音'}` : '使用设备发音'} onClick={() => playPronunciation(data.lexeme || activeScope, region, Boolean(pronunciation?.audio))}><span>◉</span><small>{region === 'us' ? '美音' : '英音'}<b>{pronunciation?.phonetic || '设备发音'}</b></small></button>; })}</div></div>{data.image ? <img className="react-dictionary-image" src={imageSrc(data.image)} alt={`${data.word} 相关课程图示`} /> : <div className="react-dictionary-image placeholder">暂无图示</div>}</div>
