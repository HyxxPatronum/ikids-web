    ...
    const state = { card:null, active:'reading', rate:1, current:-1, playbackProgress:0, speechToken:0, singlePlaying:false, previewIndex:null, dragWasPlaying:false, resumePosition:null, practiceCurrent:-1, playing:false, wordBankReviewed:false, selectedWord:null, selectedMeaning:null, selectedMeaningAnswer:null, matched:[], newMatchWord:null, matchAnimationWord:null, wordTileOrder:[], wordTileSelected:null, wordTileMatched:[], wordTileFeedback:[], wordTileLocked:false, wordTileTimer:null, wordTileCombo:0, wordTileCelebration:null, wordTileBurstShown:false, wordTileSequence:0, context:{}, contextSelected:null, contextChecked:false, contextWrong:[], contextReturned:[], contextFeedbackTimer:null, translationsVisible:{}, fullTranslateOpen:false, comprehension:{}, comprehensionChecked:false, rebuild:[], rebuildSelected:null, rebuildBankOrder:[], rebuildChecked:false, moduleStatus:{}, recording:null, recordingIndex:-1, recordingStartedAt:null, recordingTimer:null, recordingUrls:{}, recordingDataUrls:{}, recordingBlobs:{}, submittedPractice:[],scoring:{},scores:{},unrecognized:{} };
    ...
    const slug = location.pathname.startsWith('/lesson/') ? decodeURIComponent(location.pathname.split('/').filter(Boolean).pop()) : new URLSearchParams(location.search).get('lesson') || 'day001-seed';
    ...
    function wordTileCelebrationHtml(){const feedback=state.wordTileCelebration;if(!feedback||feedback.stage!=='eliminated')return '';if(state.wordTileBurstShown)return '';state.wordTileBurstShown=true;var detail=[feedback.english,feedback.meaning].filter(Boolean).join(' · ');return '<div class="word-match-burst eliminated" role="status" aria-live="assertive"><strong>Eliminate</strong>'+(detail?'<span>'+escapeHtml(detail)+'</span>':'')+'</div>';}
    function renderWordTileGame(items){
      ensureWordTileOrder(items);
      const cards=state.wordTileOrder.map(id=>{const [pair,kind]=id.split(':');const item=items.find(entry=>entry.index===pair);if(!item)return '';if(state.wordTileMatched.includes(pair))return '';const selected=state.wordTileSelected===id;const feedback=state.wordTileFeedback.includes(id);const clearing=state.wordTileFeedback.includes(`clear:${id}`);const label=kind==='word'?`单词卡：${item.english}`:`图片卡：${item.meaning||item.english}`;return `<button type="button" class="word-tile ${kind} ${selected?'selected':''} ${feedback?'mismatch':''} ${clearing?'clearing':''}" data-word-tile="${id}" aria-pressed="${selected}" aria-label="${escapeHtml(label)}" ${state.wordTileLocked?'disabled':''}>${kind==='word'?`<strong class="word-tile-text">${escapeHtml(item.english)}</strong>`:`${wordTileIllustration(item)}`}${selected?'<span class="word-tile-pick" aria-hidden="true"><svg viewBox="0 0 20 20"><path d="m4 10 4 4 8-9"/></svg></span>':''}</button>`;}).join('');
      const remaining=items.length-state.wordTileMatched.length;
      ...
      return `<section class="subsection word-tile-game" aria-labelledby="wordTileTitle"><div class="word-game-shell ${complete?'complete':''}">${wordTileCelebrationHtml()}<div class="word-game-hud"><div class="word-game-title"><span class="word-game-mark" aria-hidden="true"><svg viewBox="0 0 24 24"><rect x="3" y="5" width="8" height="8" rx="2"/><rect x="13" y="11" width="8" height="8" rx="2"/><path d="m8 17 3 3 5-6"/></svg></span><div><h2 id="wordTileTitle">Word Match</h2><p>一词一图，找出 ${items.length} 组科学搭档</p></div></div><div class="word-game-score"><strong>${state.wordTileMatched.length} / ${items.length}</strong><span>已消除</span></div></div><div class="word-game-progress" style="grid-template-columns:repeat(${Math.max(items.length,1)},1fr)" aria-hidden="true">${progress}</div><div class="word-tile-board" role="group" aria-label="单词与图片配对棋盘，剩余 ${remaining} 张卡">${cards||'<span class="word-tile-empty">全部消除！</span>'}</div><div class="word-tile-status" aria-live="polite"><div class="word-tile-status-copy"><span class="word-tile-status-icon">${wordTileStatusIcon(complete)}</span><p class="${complete?'success-copy':''}">${wordTileStatus(items)}</p></div><button class="word-game-reset" id="resetWordTiles" type="button">重新洗牌</button></div></div></section>`;
    }
    function handleWordTile(id){
      ...
      if(firstPair===pair&&firstKind!==kind){
        ...
        state.wordTileCelebration={stage:'eliminated',combo,english:String(word?.english||word?.word||''),meaning:String(word?.chinese||word?.meaning||'')};state.wordTileBurstShown=false;
    ...
    function renderWords(){
      ...
      // 结果栏：已匹配的词->含义，样式同词卡 box
      const matchedResults=state.matched.map(word=>{const pair=pairs.find(p=>p.word===word);const tone=toneFor(word);const isNew=state.newMatchWord===word?' is-new':'';return pair?`<article class="match-result-chip${isNew}" data-tone="${tone}"><strong>${escapeHtml(pair.word)}</strong><span class="match-arrow">→</span><small>${escapeHtml(pair.meaning)}</small></article>`:'';}).join('');
      const resultsBox=matchedResults?`<div class="match-results-box" aria-label="已匹配结果"><p class="match-results-title"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 12 5 5 11-11"/></svg>已配对 · ${state.matched.length} / ${pairs.length}</p>${matchedResults}</div>`:'';
      state.newMatchWord=null;
      // 匹配板：过滤掉已匹配项
      const remainingPairs=pairs.filter(pair=>!state.matched.includes(pair.word));
      const remainingMeanings=meanings.filter(pair=>!state.matched.includes(pair.word));
      const matchBoard=remainingPairs.length?`<div class="match-board"><div class="match-column">${remainingPairs.map(pair=>{const done=state.matched.includes(pair.word);return `<button type="button" class="match-choice ${state.selectedWord===pair.word?'selected':''} ${done?'matched':''}" data-tone="${toneFor(pair.word)}" data-match-word="${escapeHtml(pair.word)}" ${done?'disabled':''}>${escapeHtml(pair.word)}</button>`;}).join('')}</div><div class="match-column">${remainingMeanings.map(pair=>{const done=state.matched.includes(pair.word);return `<button type="button" class="match-choice ${state.selectedMeaning===pair.meaning?'selected':''} ${done?'matched':''}" data-tone="${toneFor(pair.word)}" data-match-meaning="${escapeHtml(pair.meaning)}" data-match-answer="${escapeHtml(pair.word)}" ${done?'disabled':''}>${escapeHtml(pair.meaning)}</button>`;}).join('')}</div></div>`:'<p class="match-board-empty">全部配对完成，去玩下方 Word Match 吧！</p>';

      $('#wordsModule').innerHTML=`<div class="panel practice-shell"><section aria-labelledby="wordBankTitle"><h2 class="section-title task-title" id="wordBankTitle"><span class="task-tag">Task 3</span><span class="task-divider" aria-hidden="true"></span><span class="task-name">Words</span></h2><p class="muted words-guide">${state.wordBankReviewed?'练习时点右上角「回看单词」可回到词卡复习。':'先记住这些单词，再开始配对练习。'}</p><div class="word-bank-box" id="wordBankBox" ${state.wordBankReviewed?'hidden':''}>${bankSlots.map(word=>word?`<article class="word-card"><span class="word-card-image" aria-hidden="true"></span><span class="word-card-copy"><strong>${escapeHtml(word.english||word.word||'')}</strong><small>${escapeHtml(word.chinese||word.meaning||'')}</small></span></article>`:'<span class="word-card empty" aria-hidden="true"></span>').join('')}</div><div class="btn-row word-bank-actions">${state.wordBankReviewed?'':`<button class="btn primary" id="toggleWordBank" type="button">开始配对 →</button>`}</div></section><div class="words-practice" id="wordsPractice" ${state.wordBankReviewed?'':'hidden'}><div class="practice-intro"><div><p class="eyebrow">Word Match</p><h2 class="section-title">把核心词和含义连起来</h2><p class="muted">先选左边的单词，再选右边的含义。正确配对会被锁定。</p></div><div class="intro-actions"><span class="badge ${state.matched.length===pairs.length?'success':''}">${state.matched.length} / ${pairs.length} 已配对</span>${state.wordBankReviewed?`<button class="link-btn" id="toggleWordBank" type="button">回看单词</button>`:''}</div></div>${resultsBox}${matchBoard}<div class="btn-row" style="margin-top:12px"><button class="btn" id="resetMatch" type="button">重置配对</button></div>${renderWordTileGame(tileWords)}<section class="subsection" id="contextPractice"></section></div></div>${moduleCompleteBox('words')}`;
    ...
    function checkContext(){const qs=contextQuestions();if(Object.keys(state.context).length<qs.length){toast('请先填完所有句子的空格');return;}const wrong=qs.map((q,i)=>state.context[i]===contextAnswer(q)?-1:i).filter(index=>index>=0);state.contextChecked=true;state.contextWrong=wrong;if(wrong.length){wrong.forEach(index=>{const word=state.context[index];if(word)state.contextReturned.push(word);delete state.context[index];});state.contextReturned=[...new Set(state.contextReturned)];renderContextPractice();state.contextReturned=[];clearTimeout(state.contextFeedbackTimer);state.contextFeedbackTimer=setTimeout(()=>{state.contextWrong=[];renderContextPractice();},2000);}else{renderContextPractice();completeWordsIfReady();}saveProgress();}
    function tryMatch(){if(!state.selectedWord||!state.selectedMeaning)return;if(state.selectedWord===state.selectedMeaningAnswer){state.matched.push(state.selectedWord);state.newMatchWord=state.selectedWord;setTimeout(completeWordsIfReady,0);}state.selectedWord=null;state.selectedMeaning=null;state.selectedMeaningAnswer=null;saveProgress();}
